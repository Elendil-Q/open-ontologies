...
- **Decision 0002 references a "decision 0004" that does not exist** in `docs/decisions/`. The
  description-logic model certificate it points at is `lean/Dl/` and is implemented; only the
  record is missing. Not fixed here, because writing someone else's decision record is not this
  agent's to do.
